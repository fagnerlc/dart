...
main(List<String> args) {
  print('16.1.1) List manipulando Objetos\n');
  ...
  print('\n16.1.2) Ordenando Objetos\n');

  List<Vendedor> vendedores = List();
  vendedores.add(Vendedor('Fagner', [
    Venda('iPhoneX', 4199.99),
    Venda('MacBookPro', 5300.00),
    Venda('Galaxy S10', 3500.25)
  ]));
  vendedores.add(Vendedor('Lorena', [
  ...
  vendedores.sort((b, a) => (a.vendas
      .fold(0, (previousValue, element) => previousValue + element.preco)
      .compareTo(b.vendas.fold(
          0,
          (previousValue, element) =>
              previousValue +
              element.preco)))); // element é sempre o valor atual
  ...
  vendedores.forEach((element) => print(
      'Vendedor #${element.nome} \t ${element.vendas.length} vendas '
  ...
  proprietarios.add(Proprietario('Fagner', 'Rua Belisário Távora, 275', [
    Carro('Renault', 'Sandero', 'FMP9435',
        Caracteristicas('Passeio', 5, 116, 'Felx'),
        multas: [
          Multa('Excesso de Velocidade', 'Gravíssima', 7),
          Multa('Estacionar em local proibido', 'grave', 5),
        ]),
    Carro('Fiat', 'Toro', 'FLC1987', Caracteristicas('Carga', 5, 106, 'Felx'),
        multas: [
          Multa('Excesso de Velocidade', 'Gravíssima', 7),
          Multa('Radar', 'Leve', 3),
        ]),
  ...
  proprietarios.sort((b, a) => a.nome.compareTo(
      b.nome)); // muda a ordem de execulção quem foi gravado por ultimo
  ...
  for (var i = 0; i < proprietarios.length; i++) {
    ...
    for (var carro in carros) {
      carro.multas.forEach((element) {
        print(
            '${proprietarios[i].nome} \t ${carro.modelo} ${carro.placa} ${carro.caracteristica.tipo} \t ${element.descricao} - ${element.pontos} pontos');
      });
    }
  }
}
